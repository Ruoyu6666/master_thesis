...
from util import DataLoaderS, masked_mae, masked_mape, metric, pinball_loss
from trainer import Optim



parser = argparse.ArgumentParser(description='PyTorch Time Series Non-Parametric Probalistic Forecasting')
#parser.add_argument('--num_quantiles', type=int, default=8, help='number of #quantiles')

parser.add_argument('--data', type=str, default='./data/others/electricity/electricity.txt', help='location of the data file')
parser.add_argument('--log_interval', type=int, default=2000, metavar='N', help='report interval')
parser.add_argument('--save', type=str, default='./model-electricityNonParaProb.pt', help='path to save the final model')
...
def train(data, X, Y, model, criterion, optim, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        for j in range(args.num_split):
            ...
            output = model(tx, id)
            output = torch.squeeze(output, 3) # [B, num_quantiles (7), num_nodes]
            if len(output.size()) < 3:
                output = output.unsqueeze(dim=0)
            ty = ty.unsqueeze(dim=2)          # [B, num_nodes, 1]
            output = output.transpose(1, 2)   # [B, num_nodes, num_quantiles]
            
            # quantitle loss
            errors = ty - output
            upper = quantiles.to(device) * errors
            lower = (quantiles - 1).to(device) * errors
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size):
    ...
    total_loss_l1 = 0
    n_samples = 0
    predict0 = None # quantile predictions
    predict = None  # pre
    test = None
    
    for X, Y in data.get_batches(X, Y, batch_size, False):  # X [B, 24, 8], Y: [B, 8]
        X = torch.unsqueeze(X, dim = 1) # [B, 1, 24, 8]
        X = X.transpose(2, 3)           # [B, 1, 8, 24]
        with torch.no_grad():
            output0 = model(X)          # [B, 7, 8, 1]

        output = torch.squeeze(output0,3)[:,3,:] # 0.5 quantiles -> mean
        
        if len(output.shape)==1:
            output = output.unsqueeze(dim=0)
        if predict0 is None and predict is None:
            predict0 = output0
            predict = output
            test = Y
        else:
            predict0 = torch.cat((predict0, output0))
    ...
    predict0 = predict0.data.cpu().numpy()
    ...
    return rse, rae, correlation, predict0, Ytest
...
def main():
    ...
    model = gthnet(args.gcn_true, args.buildA_true, args.buildH_true, args.gcn_depth, 
                   args.num_nodes, args.num_hyperedge, device = "cuda", 
    ...
    vtest_acc, vtest_rae, vtest_corr, _, _ = evaluate(Data, Data.valid[0], Data.valid[1], model, evaluateL2, evaluateL1,args.batch_size)
    ...
    #np.save(f"./predict/others/prob/predictELQ.npy", predict)
    #np.save(f"./predict/others/prob/testELQ.npy", Ytest)
